...
dependencies {
    ...
    implementation("net.aechronis:vanilla:0da3ce0") // DCFiendish/vanilla fork — Slow Falling fall-damage fix + off-thread chunk/entity mutation fixes (Crops/Saplings/TreeFeller/EnvironmentalDamage/Food/Combat) + KillShop removed + save-race/fall-damage-teleport/ore-drop/autosave fixes + EnvironmentalDamage per-tick perf fix
    implementation("net.aechronis:nodes:5090058") // DCFiendish/nodes fork — lag/port-name/flight/war-restart/ore-dupe/territory-load/partial-load/dual-town/permissions/ghost-plots/war-save/ore-sampling fixes + unbounded protect-show particle task fix + public Nodes.enableWar() for programmatic startup + war thread-safety/ore-cache-wraparound/mutate-during-iterate/town-destroy-war-cleanup fixes + Nametag per-tick perf fix + cherry-picked upstream fixes: nation-membership-index desync, stale respawn point for ex-town-members
