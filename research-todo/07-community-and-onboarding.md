...
- `Aechronis/resource-pack` is themed around Aechronis's own modern-military content (per `06-economy-and-progression.md`'s Buildings/vehicle-factory findings — AK-47/M4A1/tank/F-16/drone assets). Since this project's theme is **the First Balkan War (1912–1913)** (decided 2026-08-05), **that resource pack almost certainly isn't a fit as-is** — this project likely needs its own First Balkan War-themed resource pack (bolt-action rifles, period uniforms/faction insignia for the Balkan League states and the Ottoman Empire, horse-drawn artillery, early biplanes) or a heavily stripped/retextured fork, not adoption of Aechronis's asset set. The resource-pack branches (`template`, `a-new-millenium`, `event`) mentioned in the org's repo list are Aechronis's own branches and probably not directly relevant here beyond "if we fork their repo as a starting technical template for how resource-pack building/deployment works."
