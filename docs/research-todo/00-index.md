...
> **Status update (2026-08-06):** the map/border design flagged as open throughout this folder is now done and deployed to production — real trimmed Anvil terrain (Britain through Morocco) is live on the server, real-geodata 1911 national borders are drawn and tiled into organic territories across all ten nations, and `nodes-map` (the territory viewer) is deployed and reachable. See `HANDOFF.md` for the full current-status summary and `04-world-and-data-architecture.md`'s "RESOLVED" section for detail. Everything below that still frames this as an open question is describing the state of things before this work, kept for the design rationale, not the current status.

> **Important scope correction (2026-07-30, theme finalized 2026-08-05, revised same day):** "Aechronis" is a **separate, third-party Minestom server** — this project is a *different*, not-yet-named server that's adopting Aechronis's `nodes`/`combat`/`vanilla`/`utils` libraries as technical infrastructure (land claims, gun/melee combat, vanilla-feature reimplementation) because no better-maintained modern alternative to the old `nodes` plugin exists. **This project's own game design is explicitly not copying Aechronis's** — the decided theme is **the Agadir Crisis (1911), alternate history** — the real 1911 diplomatic standoff over Morocco, reimagined here as escalating into actual war between Germany, France, the United Kingdom, Spain, Italy, Morocco, and (for now) Switzerland, Portugal, Netherlands, and Belgium (superseded the earlier First Balkan War framing the same day it was written — same underlying arms tech either way: bolt-action rifles/Maxim guns/horse-drawn artillery, not flintlock muskets) — vs. Aechronis's own modern-military theme — so anywhere below that references Aechronis's own Discord, domain (`aechronis.net`), Territory Tier bonus curve, or Buildings/oil/vehicle-factory economy, treat that as **reference context explaining the code's origin, not a build target for this project.** The bug-level findings in `COMBAT_DEEP_DIVE.md`/`NODES_DEEP_DIVE.md`/`VANILLA_DEEP_DIVE.md` remain fully relevant regardless — this project inherits that code's bugs either way. Confirmed in `02-cross-library-integration.md`: `combat`'s `Vehicle`/`Car`/`Tank` class hierarchy is generic (tunable physics numbers, not modern-specific), so it's being reused as the base for this era's actual matériel — horse-drawn field guns, supply wagons, and the like — the vehicle bug list stays fully in scope, it's not dead code.
...
| World-gen/persistence format (Anvil chosen for the real trimmed terrain 2026-08-06, REVERTED 2026-08-25 — see `HANDOFF.md`), JVM/GC tuning, memory footprint, SLF4J binding | `minestom-server-setup/03-runtime-ops-and-logging.md`, `04-world-generation-and-persistence.md` |
...
10. [`10-asset-sourcing-and-licensing.md`](10-asset-sourcing-and-licensing.md) — **new 2026-08-25**, replacing the abandoned from-scratch Blockbench modeling workflow: policy for sourcing public/CC0 weapon/vehicle/building/uniform models, and the technical format `combat` actually expects them in.
